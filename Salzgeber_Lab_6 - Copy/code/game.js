...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); // rounds everything down
  var xEnd = Math.ceil(pos.x + size.x); //rounds everything up
  var yStart = Math.floor(pos.y);
  var yEnd = Math.ceil(pos.y + size.y);

  // Treat sides, top, bottom of level as walls
  if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

  //Check each grid position starting at yStart, xStart
  //for possible obstacles (non null values)
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  //The floor is also an obstacle --- only allow jump if on obstacle
  if (obstacle == "wall") {
